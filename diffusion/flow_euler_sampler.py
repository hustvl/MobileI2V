...
class FlowEuler:
    ...
    def sample(self, latents, guide_image, flow_score, steps=28):
        ...
        if do_classifier_free_guidance:
            ...
            #guide_image = torch.cat([guide_image, guide_image], dim=0)

        for i, t in tqdm(list(enumerate(timesteps)), disable=os.getenv("DPM_TQDM", "False") == "True"):

            # expand the latents if we are doing classifier free guidance
            latent_model_input = torch.cat([latents] * 2) if do_classifier_free_guidance else latents
            #flow_score = torch.cat([flow_score] * 2) if do_classifier_free_guidance else latents
            
            # broadcast to batch dimension in a way that's compatible with ONNX/Core ML
            timestep = t.expand(latent_model_input.shape[0])
            flow_score = flow_score.expand(latent_model_input.shape[0])
            
            #print(latents.shape)
            batch_size,_,frames,H,W = latents.shape
            ...
            #print(t)
            latents[:, :, :1, :, :] = torch.lerp(latents[:, :, :1, :, :], guide_image, 1.0)
